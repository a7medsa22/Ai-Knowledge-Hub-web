...
export const aiService = {
  ...
  summarize: async (data: SummarizeInput) => {
    const response = await api.post<SummarizeResponse>('/ai/summarize', data);
    return response.data;
  },

  // RAG Question & Answer
  ask: async (data: AskInput) => {
    const response = await api.post<AskResponse>('/ai/ask', data);
    return response.data;
  },

  // Semantic Search
  search: async (data: SearchInput) => {
    const response = await api.post<SearchResult[]>('/ai/search', data);
    return response.data;
  },

  // Legacy Q&A (Direct context)
  chat: async (data: { message: string; context?: string }) => {
    const response = await api.post<{ response: string }>('/ai/chat', data);
    return response.data;
  },

  // Extract key points
  extractKeyPoints: async (data: ExtractKeyPointsInput) => {
    const response = await api.post<ExtractKeyPointsResponse>('/ai/extract-key-points', data);
    return response.data;
  },

  // Summarize multiple documents
  bulkSummarize: async (data: BulkSummarizeInput) => {
    const response = await api.post<BulkSummarizeResponse>('/ai/bulk-summarize', data);
    return response.data;
  },
};